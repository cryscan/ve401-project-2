...
The number of incidents is denoted as $X$. We assume $\lambda$ is the average number of incidents per day and $n$ is the number of days we observe, then the probability to have $N$ incidents happen will be 
\begin{equation*}
...
\begin{equation*}
	\hat{\lambda} = \frac{X}{n}
\end{equation*}
will be an unbiased estimator for $\lambda$. The unit of $\hat{\lambda}$ is $\text{day}^{-1}$. For this period of observation, we have 
\begin{align*}
	n &= 1461,\\
	N &= 3943.
\end{align*}
...
\begin{equation*}
	\hat{\lambda} = \frac{N}{n} \approx 2.70.
\end{equation*}
...
\[H_{0}:\text{ Poisson distribution for year 2015 is the same as Poisson distribution for 4 years together.}\]
We denote the parameter $\lambda_{1}$ for 4 years together and $\lambda_{2}$ for year 2015. 
\[\lambda_{1} = 2.699,\quad \lambda_{2} = 2.726\]
...
\[\frac{|\hat{\lambda_{1}}-\hat{\lambda_{2}}|}{\sqrt{\lambda_{1}n_{1}+\lambda_{2}n_{2}}} > z_{0.025}.\]
From our calculation, we have 
\[\frac{|\hat{\lambda_{1}}-\hat{\lambda_{2}}|}{\sqrt{\lambda_{1}n_{1}+\lambda_{2}n_{2}}} = \]
...
We consider following $m$ days and the random variable for number of incidents to happen $Y$. We use $\hat{Y}$ to predict the value of $Y$.

We used number of incidents $X$ in previous $n$ to predict $Y$, the estimator $\hat{Y}$ is 
\begin{equation*}
...
\begin{align*}
	\text{Var}[Y-\hat{Y}] &= \text{Var}[Y]+\frac{m^{2}}{n^{2}}\text{Var}[\hat{\lambda}t]\\
...
\begin{figure}[htbp]
	\centering
	\includegraphics[]{}
	\caption{Accumulated number of incidents by observation and 95\% confidence prediction interval in 2019.}
\end{figure}
\fi
